    ...
    override fun customViewHolder(parent: ViewGroup, viewType: Int) =
        when (status) {
            HistoryType.PENDING.statusCode ->
                PendingViewHolder(onItemClickListener, inflateView(R.layout.item_pending, parent))
            HistoryType.APPROVE.statusCode ->
                ApproveViewHolder(onItemClickListener, inflateView(R.layout.item_approve, parent))
            HistoryType.REJECT.statusCode ->
                RejectViewHolder(onItemClickListener, inflateView(R.layout.item_reject, parent))
            HistoryType.FINISH.statusCode ->
                FinishViewHolder(onItemClickListener, inflateView(R.layout.item_done, parent))
            else -> Any()
        }
